...
function Dataset(generatorConfig::ExpressionGeneratorConfig, trees::Vector{Node{T}}) where T <: Number
    ...
    return Dataset{T}(generatorConfig, length(trees), trees, generatorConfig.ops, Matrix{T}(undef, 0, 0), Matrix{T}(undef, 0, 0), Matrix{Bool}(undef, 0, 0), Matrix{T}(undef, 0, 0))
end

function Dataset(generatorConfig::ExpressionGeneratorConfig, trees::Vector{Node{T}}, eval_x::Matrix{T}, eval_y::Matrix{T}) where T <: Number
    @assert length(trees) == size(eval_y, 1)

    # Create a Dataset with trees and evaluated values
    return Dataset{T}(generatorConfig, length(trees), trees, generatorConfig.ops, eval_x, eval_y, Matrix{Bool}(undef, 0, 0), Matrix{T}(undef, 0, 0))
end

function Dataset(generatorConfig::ExpressionGeneratorConfig, trees::Vector{Node{T}}, eval_x::Matrix{T}, eval_y::Matrix{T}, onehot::BitArray, consts::Matrix{T}) where T <: Number
