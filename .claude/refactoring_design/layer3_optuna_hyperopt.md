# Layer 3: Optuna Hyperparameter Optimization (`optuna_hyperopt.py`)

## Purpose
Orchestrates the complete Optuna study lifecycle including study management, hyperparameter sampling, and trial execution. Combines what was previously Layers 3 and 4 into a single cohesive class.

## Rationale for Combining Layers 3+4

### Why One Class Instead of Two?

1. **Tight coupling**: The objective is never used independently of the study
2. **No reusability**: The objective is custom to our SR workflow - won't be reused elsewhere
3. **Shared state**: Both need config, executor, signal manager
4. **Natural hierarchy**: The objective is logically a helper method of the study
5. **Simpler mental model**: "Running an Optuna study" → one class, not two

### What We Gain:
- ✅ Clearer ownership: all Optuna logic in one place
- ✅ Less boilerplate: no passing state between classes
- ✅ Direct access: objective can access study state directly
- ✅ Easier to understand: complete workflow in one file

### What We Lose (and why it's okay):
- ❌ Can't swap objective independently → We never would anyway
- ❌ Larger class (~640 lines) → Still reasonable with clear sections
- ❌ Multiple responsibilities → Study and objective are inherently coupled in Optuna
...
class OptunaHyperoptRunner:
    ...
    Handles:
    - Study creation, configuration, and lifecycle
    ...
    def __init__(self, config_path: str, node_id: int = None, total_nodes: int = None):
        ...
        self.executor = DistributedTrialExecutor(node_id, total_nodes, self._get_coord_dir())
    ...
    def _run_master_optimization(self, resume: bool = False):
        ...
        Creates/loads study, enqueues baseline, runs optimization loop,
        prints results.
        """
        # Create study
    ...
    def _reconstruct_hyperparameters(self, trial: optuna.Trial,
                                    saved_params: Dict) -> Tuple[ModelSettings, NeuralOptions, MutationWeights]:
        """
    ...
    def _reconstruct_objects(self, params: Dict) -> Tuple[ModelSettings, NeuralOptions, MutationWeights]:
...
- All Optuna logic in one place
- Public API: `__init__()` and `optimize()`
- Everything else is private implementation

### ✅ Clear Sections
- Public API (2 methods)
- Master optimization (study management, objective)
- Worker loop
- Hyperparameter sampling
- Trial management (resumption)
- Study setup (sampler, pruner, baseline)
- Results printing
- Entry point (main)

### ✅ Direct Access
- Objective method can access `self.study`, `self.config`, etc.
- No passing state between classes
- Natural composition with executor and signal manager

### ✅ Still Testable
- Each private method can be tested independently
- Clear separation of concerns within the class
- Factory functions for runner creation

## Class Size

**Total: ~640 lines** (well-organized with clear sections)

- Public API: ~30 lines
- Master optimization: ~150 lines
- Worker loop: ~20 lines
- Hyperparameter sampling: ~200 lines
- Trial management: ~40 lines
- Study setup: ~80 lines
- Results: ~80 lines
- Utilities + entry point: ~40 lines

## Benefits Over Two Classes

1. **No artificial boundary** between study and objective
2. **Direct state access** (no need to pass everything)
3. **Easier to understand** complete workflow
4. **Less boilerplate** and indirection
5. **Natural structure** - objective is clearly a helper method

## Trade-offs

**We accept:**
- Larger class (~640 lines, but well-organized)
- Multiple responsibilities (study + trial, but they're coupled anyway)
