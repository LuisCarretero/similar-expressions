# Layer 2: Distributed Execution (`distributed_executor.py`) - REVISED

## Purpose
Coordinates distributed execution across master and worker nodes. Generic - no SR-specific code.

## Key Design Decisions (Based on Feedback v2)

### ✅ No batch-wise synchronization
- Workers send incremental results, master scans asynchronously
- Master reports each batch as it completes (master or worker)

### ✅ Report EACH batch individually
- Master reports after each of its batches
- Master also reports each worker batch when discovered
- NO final report - Optuna only expects incremental reports

### ✅ Use interruption_manager everywhere
- Both master and worker use same interruption_flag
- No `_check_shutdown_signal()` or shutdown flag file needed
- Interruption manager handles SLURM signals for all processes
...
### ✅ Final flag for worker completion
- Workers set `final=True` when done (no new data sent)
- Master waits for all workers to set final flag
- Used only for synchronization, not data
...
class DistributedTrialExecutor:
    ...
    def execute_trial_with_batching(self, trial_params, all_equations, batch_size,
        ...
        try:
            ...
            return all_master_results + final_worker_results

        finally:
            # Clean up coordination files
            self._cleanup_trial_files(trial_id)

    def _report_new_worker_batches(self, trial_id, worker_batches_reported,
                                   batch_size, report_callback):
        """
    ...
    def _wait_and_report_remaining_worker_batches(self, trial_id, worker_batches_reported,
                                                  batch_size, report_callback,
                                                  interruption_flag):
        """
    ...
    def cleanup(self):
        """Cleanup on shutdown (no shutdown signal needed)."""
        pass  # Interruption manager handles shutdown for all processes
```

## Communication Flow Per Batch (Revised)

```
Master                          Worker 1                    Worker 2
  |                                |                           |
  |-- run batch 0                  |-- run batch 0            |-- run batch 0
  |-- report(master_batch_0)       |-- send results           |-- send results
  |                                |                           |
  |-- scan workers                 |                           |
  |-- report(worker1_batch_0)      |                           |
  |-- report(worker2_batch_0)      |                           |
  |                                |                           |
  |-- run batch 1                  |-- run batch 1            |-- run batch 1
  |-- report(master_batch_1)       |-- send results           |-- send results
  |                                |                           |
  |-- scan workers                 |                           |
  |-- report(worker1_batch_1)      |                           |
  |-- report(worker2_batch_1)      |                           |
  |                                |                           |
  | (master done)                  |                           |
  |                                |-- run batch 2            |-- run batch 2
  |                                |-- send results, final    |-- send results, final
  |-- scan workers                 |                           |
  |-- report(worker1_batch_2)      |                           |
  |-- report(worker2_batch_2)      |                           |
  |                                |                           |
  |-- detect all workers done      |                           |
  |-- mark trial processed         |                           |
  ↓                                ↓                           ↓
```
...
### ❌ Removed
- `_check_shutdown_signal()` - Use interruption_manager instead
- `shutdown.signal` file - Not needed
- `signal_shutdown()` - Not needed
- Final report in master - Only incremental reports

### ✅ Changed
- **Master reports each batch individually** (master and worker batches)
- **Uses interruption_manager everywhere** (same flag for all processes)
- **Final flag purely for synchronization** (no new data sent)
- **Track and report worker batches incrementally** as they arrive

### ➕ Added
- `_report_new_worker_batches()` - Scan and report new worker batches
- `_wait_and_report_remaining_worker_batches()` - Continue reporting after master finishes
- `_get_worker_results()` - Get current worker results (non-blocking)
- `_is_worker_done()` - Check if worker set final flag
